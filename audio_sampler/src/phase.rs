...
#[inline]
fn align_saw(speed: f64, length: f64, x: f64, y: f64) -> f64 {
    let shift = y - x * speed;
    shift
}

#[inline]
fn align_tri(speed: f64, length: f64, x: f64, y: f64, sec: bool) -> f64 {
    let shift = if !sec {
        y / speed - x
    } else {
...
impl Phase for Tri {
    ...
    fn update_length(&mut self, x: f64, l: f64) {
        ...
        self.length = l;
        self.shift = align_tri(self.speed, l, x, y1, sec1);
    }
    fn to_tri(&self, x: f64) -> PhaseEnum {
...
impl Phase for Saw {
    ...
    fn update_length(&mut self, x: f64, l: f64) {
        ...
        self.length = l;
        self.shift = align_saw(self.speed, l, x, y);
    }
    fn to_tri(&self, x: f64) -> PhaseEnum {
        let y = self.calc(x);
        PhaseEnum::Tri(Tri::new_shifted(
            self.speed,
            self.length,
...
mod test {
    ...
    fn test_essential() {
        ...
        for i in 0..(3 * l) {
            ...
            p4.update_speed(x, 1.0);
            p4.update_length(x, l as f64);
